...
import { sortedUniq, keys, uniq, sortBy } from "lodash";
import prettier from "prettier";
import parserBabel from "prettier/parser-babel";
import CodeMirror from "@uiw/react-codemirror";
import { json } from "@codemirror/lang-json";
import { walkJsonSchema7, deepCloneAndMergeLeft } from "./utils";
...
export function App() {
  ...
  const diffSchemas = () => {
    ...
    uniqueKeys.forEach((dataRef) => {
      ...
      if (JSON.stringify(s1.type) !== JSON.stringify(s2.type)) {
        schemaDiffs.push(
          `${dataRef} has conflicting types: ${s1.type} and ${s2.type}`
        );
      }
      if ((s1.enum && !s2.enum) || (s2.enum && !s1.enum)) {
        schemaDiffs.push(
          `${dataRef} has conflicting types. One is an Enum and one is not`
        );
        return;
      }
      if (s1.enum && s2.enum) {
        if (
          JSON.stringify(sortedUniq(sortBy(s1.enum))) !==
          JSON.stringify(sortedUniq(sortBy(s2.enum)))
        ) {
  ...
  const mergeSchemas = () => {
    const clonedSchema = deepCloneAndMergeLeft(schema1, schema2);

    setDiffs(
      prettier.format(JSON.stringify(clonedSchema) || "", {
        parser: "json",
        plugins: [parserBabel],
      })
    );
